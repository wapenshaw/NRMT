...
function newCollection(sub = '', weight = 1, color = 'white') {
	...
	weightInput.onkeydown = function (e) {
		if (e.key.match('[0-9.]') || e.key == 'Backspace') {
			changesMade = true; // Ensure changesMade is true if a relevant key is pressed
			...
			if (future.match('^([0-9]+|([0-9]+.[0-9]+))$')) {
				...
				// Removed 'changesMade = false;' to prevent unsaved data
			}
			return true;
		}
		return false;
...
function onLoad(item) {
	if (item && item.index && Array.isArray(item.index)) {
		item.index.forEach((node) => {
			newCollection(node.subreddit, node.weight, node.color);
		});
	} else {
		console.log("NRMT: No valid 'index' data found in storage or storage is empty.", item);
	}
}
...
function saveAndSync() {
	...
	browser.storage.sync.set({ index }).then(null, console.error); // Changed alert to console.error
}

browser.storage.sync.get('index').then(onLoad, console.error); // Changed alert to console.error
